...
class ConfigSettings:
    def __init__(self, commandLineArguments):
        ...
        if commandLineArguments.getAsk():
            self.askUser()
...
class CommandLineArguments:
    def __init__(self):
        ...
        self.parser.add_argument(
            "-p",
            "--profile",
            help="Specify which profile name to store settings in.",
            default="default",
        )
        self.parser.add_argument(
            "-u",
            "--user",
            help="Specify username to user. Don't specify any domain information",
            default=getpass.getuser(),
        )
        self.parser.add_argument(
            "-d",
            "--domain",
            help="Specify domain. Defaults to the userdomain environment and if not polhus",
            default=os.environ.get("userdomain", "polhus"),
        )
        self.parser.add_argument(
            "-a",
            "--ask",
            help="Ask user for all values. Defaults from other command line arguments",
            action="store_true",
        )
        self.parser.add_argument(
            "-td",
            "--tokenDuration",
            help="Token duration in seconds. Default is 3600 which is the default in AWS, but generally speaking longer durations are more convenient.",
            default="3600",
        )
        self.parser.add_argument(
            "-f",
            "--filter",
            help="Filter for returned role values. Specify full name (or unique match) to avoid selecting role and login directly.",
            default="",
        )
...
class AwsConfigFile:
    def __init__(
        self,
        profile,
        configfile=os.getenv("AWS_SHARED_CREDENTIALS_FILE", "~/.aws/credentials"),
        region="eu-west-1",
        outputformat="json",
    ):
    ...
    def update(self, credentials):
        ...
        config.set(
            self.__profile, "aws_access_key_id", token["Credentials"]["AccessKeyId"]
        )
        config.set(
            self.__profile,
        ...
        config.set(
            self.__profile, "aws_session_token", token["Credentials"]["SessionToken"]
        )
...
try:
    ...
    print(
        "An exception occurred using NTLM negotiation. retrying with post to sts.rootdom.dk"
    )
    payload = {
        "UserName": settings.getUsername(),
        "Password": settings.getPassword(),
        "optionForms": "FormsAuthentication",
    }
    session.auth = None
    session.get(url=idpentryurl, headers=headers, verify=sslverification)
    response = session.post(
        url=idpentryurl, headers=headers, verify=sslverification, data=payload
    )
    assertion = getAssertionFromResponse(response)
    root = ET.fromstring(base64.b64decode(assertion))
    print(root)
...
if len(awsroles) > 1:
    ...
    print(
        "No roles returned. (If you have provided a filter you might test the unfiltered result."
    )
    quit()
...
token = conn.assume_role_with_saml(
    ...
    DurationSeconds=settings.getTokenDuration(),
)

print(token)
# Write the AWS STS token into the AWS credential file
awsConfigFile = AwsConfigFile(settings.getProfile())
awsConfigFile.update(token)

# Give the user some basic info as to what has just happened
print("\n\n----------------------------------------------------------------")
print(
    "Your new access key pair has been stored in the AWS configuration file {0} under the saml profile.".format(
        awsConfigFile.filename
    )
)
